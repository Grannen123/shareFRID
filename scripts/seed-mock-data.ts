/**
 * Seed script för Grannfrid mock-data
 *
 * Kör med: npx tsx scripts/seed-mock-data.ts
 *
 * Skapar:
...
if (!supabaseUrl || !supabaseServiceKey) {
  console.error("Saknar VITE_SUPABASE_URL eller SUPABASE_SERVICE_ROLE_KEY");
  console.log("Sätt dessa i .env filen och kör igen.");
  process.exit(1);
}
...
async function seedMockData() {
  ...
  if (profileError || !profiles?.length) {
    console.error("❌ Kunde inte hitta någon profil. Logga in först i appen.");
    process.exit(1);
  }
  ...
  console.log(`✅ Använder workspace: ${workspaceId}`);
  console.log(`✅ Ansvarig konsult: ${consultantId}\n`);
  ...
  for (let i = 0; i < CUSTOMERS.length; i++) {
    ...
    const customerNumber = `K-${String(i + 1).padStart(3, "0")}`;
    ...
    if (error) {
      console.error(
        `❌ Fel vid skapande av ${customerData.name}:`,
        error.message,
      );
      continue;
    }
  ...
  for (const customer of createdCustomers) {
    ...
    const { data: agreement, error } = await supabase
      ...
      .insert({
        ...
        fixed_amount:
          config.type === "fixed" ? (config as any).fixed_amount : null,
    ...
    if (error) {
      console.error(
        `❌ Fel vid skapande av avtal för ${customer.name}:`,
        error.message,
      );
      continue;
    }
    ...
    console.log(`  ✓ ${customer.name}: ${config.type} avtal`);
  }
  ...
  for (const customer of createdCustomers) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (error) {
        console.error(`❌ Fel vid skapande av ärende:`, error.message);
        continue;
      }
  ...
  console.log("═".repeat(50));
  console.log(`  Kunder:           ${createdCustomers.length}`);
  console.log(`  Avtal:            ${createdAgreements.length}`);
  console.log(`  Kundkontakter:    ${totalCustomerContacts}`);
  console.log(`  Ärenden:          ${createdAssignments.length}`);
  console.log(`  Ärendekontakter:  ${totalAssignmentContacts}`);
  console.log("═".repeat(50));
  console.log("\n✨ Mockdata skapad! Ladda om appen för att se datan.");
}
