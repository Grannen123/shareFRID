...
**Version:** 4.1 (implementationssynkad)
**Datum:** 2026-01-20
...
- **Auth + profil** – Supabase Auth, profilinställningar, profillista för tilldelning/visning
- **Kunder + avtal** – CRUD, sök + paginering, aktivt avtal per kund, indexeringsdatum
- **Uppdrag + journal** – CRUD, journal med redigera/arkivera/fastnåla, timmar + fakturatext
- **Tidsregistrering** – Skapas från journal, timbank-split + extraarbete, billable-flagga
- **Uppgifter** – Global lista + kund/uppdrag, tilldelning eller otilldelat, statusväxling
- **Kontakter** – Fristående/kund/uppdrag, fakturamottagare-flagga
- **Anteckningsbok** – Quick notes med sök, koppling till kund/uppdrag (valfri tidpunkt)
- **Kundanteckningar** – Pinnade anteckningar per kund
- **Filer** – Uppladdning per kund/uppdrag, validerade filtyper, signed URL-nedladdning
- **Dashboard** – KPI:er, indexeringsvarningar, mina uppgifter, aktivitet från journal/uppgifter/tid
- **Fakturering** – Periodöversikt, batch-statusflöde, CSV-export
- **Kunskapsbank** – Artiklar (HTML), kategorier + taggar, endast publicerade i listan

### 2.2 Planerad funktionalitet (se ROADMAP.md)

- Fakturering 2.0 (Fortnox-integration, automatiserad export, förbättrad "Att fakturera"-pipeline)
...
| **hourly_rate_evening ej i UI**      | Fältet finns i datamodellen men exponeras inte i gränssnittet.         |
...
| **withTimeout() standard**           | De flesta Supabase-queries wrappas för att undvika hängande UI.        |
...
- `billing_advance` är en flagga för förskott (ingen automatisk logik i appen)
- `billing_month` anger faktureringsmånad för årsvis fastpris (valfritt)
- Extraarbete faktureras till förbestämt timpris (`hourly_rate`)
- **Obligatoriska fält:** `fixed_amount`, `period` (+ `hourly_rate` för extraarbete)
...
- Validering: 25 MB max, whitelist av MIME/filändelser, blocklista för farliga ändelser
- Filnamn saneras för att undvika path traversal
- Nedladdning sker via signed URL (kort TTL)
...
profiles (1) ─────< (N) customers (responsible_consultant_id)
profiles (1) ─────< (N) tasks / journal_entries / time_entries / customer_notes / quick_notes / files / billing_batches / knowledge_articles
...
journal_entries (1) ─────< (N) files (schema, ej i UI)

agreements (1) ─────< (N) time_entries
billing_batches (1) ─────< (N) time_entries (via export_batch_id)
...
| role                  | ENUM    | admin/consultant/readonly |
| default_hourly_rate   | NUMERIC | Standard timpris          |
| notifications_enabled | BOOLEAN | App-aviseringar           |
| email_notifications   | BOOLEAN | E-postaviseringar         |
...
| billing_advance      | BOOLEAN | Förskott (flagga)               |
| billing_month        | INTEGER | Faktureringsmånad (år)          |
| hourly_rate_evening  | NUMERIC | Kvällstaxa (ej i UI)            |
| notice_period_months | INTEGER | Uppsägningstid (ej i UI)        |
| auto_renewal         | BOOLEAN | Auto-förlängning (ej i UI)      |
...
| content_type      | ENUM      | text/tiptap_json (app använder text) |
| is_pinned         | BOOLEAN   | Fastnålad                            |
| entry_type        | ENUM      | call/email/meeting/site_visit/note   |
| is_archived       | BOOLEAN   | Arkiveringsflagga                    |
| archived_at       | TIMESTAMP | Arkiverad tid                        |
| archived_by       | UUID      | FK till auth.users                   |
...
| created_by    | UUID      | FK till auth.users             |
| completed_at  | TIMESTAMP | Tidpunkt för slutförande       |
...
| customer_id          | UUID    | FK till customers (valfritt)       |
| assignment_id        | UUID    | FK till assignments (valfritt)     |
| is_processed         | BOOLEAN | Kopplad eller ej                   |
| processed_journal_id | UUID    | FK till journal_entries (valfritt) |
| created_by           | UUID    | FK till auth.users                 |
...
| customer_id          | UUID    | FK till customers (valfritt)   |
| assignment_id        | UUID    | FK till assignments (valfritt) |
| contact_type         | ENUM    | customer/assignment/standalone |
| is_invoice_recipient | BOOLEAN | Fakturamottagare               |
| name, role           | TEXT    | Namn och titel/roll            |
| email, phone         | TEXT    | Kontaktuppgifter               |
| address, notes       | TEXT    | Adress/anteckningar            |
...
| customer_id      | UUID    | FK till customers (valfritt)       |
| assignment_id    | UUID    | FK till assignments (valfritt)     |
| journal_entry_id | UUID    | FK till journal_entries (valfritt) |
| file_name        | TEXT    | Originalnamn                       |
| file_path        | TEXT    | Storage path                       |
| file_size        | INTEGER | Storlek                            |
| mime_type        | TEXT    | MIME-typ                           |
| uploaded_by      | UUID    | FK till auth.users                 |
...
| ---------------------- | --------- | ---------------------------- |
| id                     | UUID      | Primärnyckel                 |
| batch_id               | TEXT      | Batch-id (B-YYYYMM-...)      |
| customer_id            | UUID      | FK till customers            |
| period_year            | INTEGER   | Period (år)                  |
| period_month           | INTEGER   | Period (månad)               |
| status                 | ENUM      | draft/review/exported/locked |
| total_amount           | NUMERIC   | Totalsumma                   |
| exported_at            | TIMESTAMP | Exporterad tid               |
| exported_by            | UUID      | FK till auth.users           |
| fortnox_invoice_number | TEXT      | Fortnox (framtida)           |
| notes                  | TEXT      | Interna noteringar           |
...
| id           | UUID    | Primärnyckel                 |
| title        | TEXT    | Titel                        |
| content      | TEXT    | HTML-innehåll (saneras i UI) |
| category     | ENUM    | knowledge/policy/routine     |
| tags         | TEXT[]  | Taggar                       |
| is_published | BOOLEAN | Publicerad eller ej          |
| created_by   | UUID    | FK till auth.users           |
| updated_by   | UUID    | FK till auth.users           |
...
| id       | UUID | Primärnyckel     |
| name     | TEXT | Namn             |
| location | TEXT | Plats (valfritt) |

#### timebank_current_status (view)

| Fält                   | Typ     | Beskrivning        |
| ---------------------- | ------- | ------------------ |
| agreement_id           | UUID    | FK till agreements |
| customer_id            | UUID    | FK till customers  |
| included_hours         | INTEGER | Inkluderade timmar |
| hours_used_this_period | NUMERIC | Förbrukat i period |
| hours_remaining        | NUMERIC | Kvar i period      |
| period                 | ENUM    | monthly/yearly     |

_Not: De flesta tabeller har även `created_at` och `updated_at`._
...
Används som standard i data-hooks för att undvika hängande UI.
...
- **Skapande av kund:** använder `profile.workspace_id`, fallback till workspace "Göteborg"
- **RLS filtrerar på** `auth.uid()`, inte workspace_id
- **Full transparens** – alla ser all data (inkl. läsning av profiler)
...
- Profile-tabellen: läsning för autentiserade (tilldelning/visning), uppdatering begränsad till egen profil
...
- Periodöversikt över oexporterade time_entries per kund
- Skapar billing_batches per kund/period
- Statusflöde: `draft` → `review` → `exported` → `locked`
- Exporterar CSV från batch-detalj (för Fortnox-import)
- Markerar time_entries som exporterade vid `exported`
...
| Kvällspris i UI           | Ej mål   | Fältet exponeras inte      |
...
| ---------- | ------- | ------------------------------------------------------- |
| 2026-01-20 | 4.1     | Synkad med implementation (datamodell, flöden, billing) |
